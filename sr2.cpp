...
bool saveToPythonScript(Resultsf r, Resultsf rf, string s) {
  ofstream fout(s);
  if (fout.is_open()) {
    fout << "#!/bin/python3"
...
vector<ModelVariablesf> interpolate(Results r, vector<ld> timestamps) {
  vector<ModelVariablesf> ret;
  for (ll i = 0; i < timestamps.size(); i++) {
    ld now = timestamps[i];
    for (ll j = 1; j < r.t.size(); j++) {
      if (r.t[j - 1] <= now && r.t[j] >= now) {
        ModelVariablesf solved;
        double A = r.u[j - 1].preys, B = r.u[j].preys, a = r.t[j - 1],
               b = r.t[j], c = now;
        solved.preys = A + (B - A) * (c - a) / (b - a);
        A = r.u[j - 1].predators;
        B = r.u[j].predators;
        solved.predators = A + (B - A) * (c - a) / (b - a);
        ret.push_back(solved);
        break;
      }
    }
  }
...
int main() {
  ...
  RES.t.resize(rf.t.size());
  RES.u.resize(rf.u.size());
  for (int i = 0; i < 10000; i++ ){
  auto ans = interpolate(group[i],rf.t);
  for(int j = 0; j < ans.size(); j++){
    RES.t[j] = rf.t[j];
    RES.u[j].preys += ans[j].preys;
    RES.u[j].predators += ans[j].predators;
  }
  }
  for (int i = 0; i< rf.t.size(); i++){
    RES.u[i].preys /= 10000;
    RES.u[i].predators /= 10000;
  }

  if (saveToPythonScript(RES, rf, s))
    runPythonScript(s);
  return 0;
}